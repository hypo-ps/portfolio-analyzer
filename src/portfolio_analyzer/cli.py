...
from portfolio_analyzer.report import ScoredStock, build_report
...
def _load_previous_states(output_dir: Path, today: dt.date) -> dict[str, str]:
    """Return {symbol: prev_decision} from the most recent prior JSON in output_dir
    (D-BT17). Returns empty dict (defaults to HOLD) if none found or stale."""
    ...
    for entry in data.get("stocks", []):
        ...
        if sym and dec in strategy.VALID_STATES:
            states[sym] = dec
    logging.info("Seeded prev_state from %s (%d symbols).", latest_path.name, len(states))
    return states
...
def run_analysis(out_file: Path | None, do_refresh: bool = True) -> None:
    ...
    today = dt.date.today()
    prev_states = _load_previous_states(
    ...
    for symbol in holding_symbols:
        ...
        result = strategy.decide(prev, metrics, scored.decision)
        scored_stocks.append(
            ScoredStock(
                metrics=metrics, scored=scored, sector=sector_map.get(symbol, "UNKNOWN"),
                decision=result.decision, prev_state=prev,
            )
        )
